...
export default function SharedResultsPage() {
  ...
  useEffect(() => {
    if (!run_id) return
    fetch(`/api/score?run_id=${encodeURIComponent(run_id)}`)
      .then(res => {
  ...
  if (error) return (
    <div className="max-w-md mx-auto mt-20 bg-red-950/50 border border-red-800 rounded-xl p-6 text-center space-y-3">
      <div className="text-red-400 font-medium">Run not found</div>
      <div className="text-zinc-500 text-sm">{run_id}</div>
      <button onClick={() => router.push('/runs')} className="text-sm text-zinc-400 hover:text-white transition-colors">
        ← View all runs
      </button>
    </div>
  )
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <button onClick={() => router.push('/runs')} className="text-xs text-zinc-600 hover:text-zinc-400 transition-colors">
              ← All runs
            </button>
            <span className="text-zinc-700 text-xs">/</span>
            <span className="text-xs text-zinc-600 font-mono">{run_id}</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Results</h1>
          <p className="text-zinc-500 text-sm mt-1">
            {candidates.length} candidates · {roleAnalysis?.onet_name ?? 'Engineering'} · {scoringConfig?.onet_code}
          </p>
        </div>
      ...
      {tab === 'all' && hiddenGems.length > 0 && (
        <div className="flex items-center gap-3 bg-purple-950/30 border border-purple-800/40 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-purple-300">
            ...
            strong signals with incomplete data.
          </p>
        </div>
      )}
